...
- Requisitos no catálogo: **0**
- Com implementação marcada: **3**
- Com teste marcado: **0**

## Inconsistências

- RF03.2 aparece no código mas não existe no catálogo de requisitos (tag órfã).
- RF03.4 aparece no código mas não existe no catálogo de requisitos (tag órfã).
- RF03.5 aparece no código mas não existe no catálogo de requisitos (tag órfã).
